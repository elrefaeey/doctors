...
interface ImageUploaderProps {
  ...
  storagePath: string; // e.g., 'doctors/doctorId' or 'uploads/general'
  maxSizeMB?: number;
    ...
    <div className={`space-y-4 ${className}`}>
      ...
      {!error && !success && (
        <p className="text-xs text-slate-500 dark:text-slate-400 text-center">
          {language === 'ar'
            ? 'سيتم رفع الصورة مباشرة على Firebase Storage'
            : 'Image will be uploaded directly to Firebase Storage'}
        </p>
      )}
    </div>
  );
